...
1. **제목**: 종목명/지수 키워드를 앞 12자 이내에 강제. 브랜드 키워드("12시에 만나요") 제목 노출 금지. **3안 생성 후 키워드 매트릭스로 평가**. ⚠️ **v2.1(2026-06-10): "유튜브 원제 그대로"(5/27) 정책 폐기 — SEO 제목 사용, 원제는 부록에만.**
2. **본문**: `## 📍 이 글에서 다루는 것`(목차 3~5줄) + `## 🔗 관련 종목 한눈에`(3~5개 표) + **`## ❓ 오늘의 용어`(v2.1, 질문형 H2 — 롱테일 검색 자산)** 섹션 신설. 최저 분량 2,500자.
2-1. **[v2.1] 태그 8~10개** (30개 룰 폐기): 실검색어만, 12자 이상 조합형 태그 금지. **유튜브 설명란·고정댓글 링크는 네이버 블로그 URL만** (GitHub 금지, 가이드 §5).
