...
`references/tone-book.md` (v3) — 영상 리뷰 포맷 구조, 제목=원제 룰, 태그 30개 템플릿,
...
1. **제목 (v3, 2026-06-11 사용자 확정)**: **유튜브 원제 그대로** (이모지 포함, 변형 금지). SEO 제목안은 seo.md 참고용으로만 생성. 검색 키워드는 첫 100자·H2·태그에서 확보.
2. **본문 (v3): 영상 리뷰 포맷** — 화자는 "영상을 본 리뷰어". 구조(톤북 §3): 도입(리뷰어 시점) → `## ⭐ 오늘 영상 한줄평`(★n/5) → `## 📍 이 리뷰에서 다루는 것` → `## 🎬 코너별 리뷰`(h3 코너 2~4개, 코너마다 내 반응) → `## 💬 오늘의 명대사`(확인된 문구만, 멘트 창작 금지) → `## 📊 영상 속 시장 데이터 한눈에`(수치는 전부 이 보조 박스로) → `## 👍 이런 분에게 추천` → `## ❓ 영상에 나온 용어`(질문형 H2) → `## 💡 내 감상`. 톤 = 드라마틱 스토리텔링 + 커뮤니티 유머(어그로 금지). 최저 분량 2,500자.
2-1. **[v3] 태그 정확히 30개** (8~10개 룰 폐기, 사용자 확정): 12자 이상 조합형 금지, 앞 5개에 종목/지수 3개+. **유튜브 설명란·고정댓글 링크는 네이버 블로그 URL만** (GitHub 금지, 가이드 §5).
2-2. **[v2.1] 내용 룰**: 🧠 투자 심리 섹션은 **주 2회(월·목)만**, 그날 실제 시장 행동과 결합 필수. 💡 내 생각은 **1문단(3~5문장) 1인칭 판단**으로 확대. 심리 없는 날은 psychology PNG 생략(필수 PNG: market/summary). 상세는 톤북 §3 v2.1 블록.
...
**copy_tool은 v2 포맷으로 생성** (`output/{date}_copy_tool_v2.html`): ①제목(원제) 1버튼 ②본문 통복사 1버튼(text/html) ③이미지 각 1버튼 ④태그 30개 쉼표 통복사 ⑤유튜브 설명란·고정댓글.
...
- **SEO (v3)**: 발행 제목 = 유튜브 원제 그대로. SEO 제목 후보 3안은 seo.md 참고용으로만. 메타 디스크립션 80자 이내
...
**개수 규칙 (2026-06-24 갱신): PNG는 5~6장 (이전 "3~5장 가변"에서 상향 — 사용자 확정)**
- 코너별 리뷰 기준 키: `summary`(TL;DR) / `news`(① 시황) / `flows`(② 수급·종목) / `gwangsoo`(③ 고정 코너) / `market`(📊 데이터 박스) / `outlook`(🔭 전망). 그날 코너에 맞게 5~6개.
- 코너가 적은 날도 최소 5장. `insight`는 데이터에만 두고 이미지 생성 안 함(본문 텍스트로만).
- **톤: 네온(글로우) OFF** — hero 숫자는 글로우 없이 소프트 화이트(#F1F5F9→#CBD5E1), 섹션 컬러는 accent bar·칩에만. generate_infographics.py에 반영(글로우 stdDeviation 0).
